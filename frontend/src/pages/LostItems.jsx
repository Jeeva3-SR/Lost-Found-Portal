    ...
    useEffect(() => {
        const fetchItems = async () => {
            try {
                ...
                const { data } = await axios.get('http://localhost:5000/api/items?type=lost', config);
                setItems(data);
            } catch (error) {
        ...
        <Layout>
            <div className="max-w-6xl mx-auto">
                <header className="mb-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div>
                        <h1 className="text-3xl font-extrabold text-primary mb-2">Lost Items Gallery</h1>
                        <p className="text-slate-500 font-medium text-sm">Browse missing items and help return them to their owners.</p>
                    </div>
                ...
                {loading ? (
                    <div className="flex justify-center items-center h-64 font-bold text-slate-300">Loading lost items...</div>
                    ...
                    <div className="bg-white rounded-[2.5rem] p-16 text-center border border-slate-100 shadow-sm">
                        ...
                        <h3 className="text-2xl font-bold text-primary mb-2">No lost items found</h3>
                        <p className="text-slate-400 mb-0 max-w-xs mx-auto text-sm font-medium">Try adjusting your search or check back later.</p>
                    </div>
                ) : (
                            ...
                            <div key={item._id} className="bg-white rounded-[2.5rem] border border-slate-100 overflow-hidden shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all group">
                                <div className="h-56 bg-slate-100 relative">
                                    ...
                                    <div className="absolute top-6 left-6">
                                        <span className="bg-red-500 text-white px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-[0.15em] shadow-lg shadow-red-500/20">
                                            Lost
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="p-8">
                                    ...
                                    {item.reporter?._id && (
                                        <button 
                                            ...
                                            className="w-full bg-slate-900 text-white py-4 rounded-2xl text-sm font-bold flex items-center justify-center gap-2 hover:bg-black transition-all group/btn shadow-lg shadow-slate-900/10"
                                        >
                                            I Found It <ArrowRight size={18} className="group-hover/btn:translate-x-1 transition-transform" />
                                        </button>
                                    )}
                                </div>
                            </div>
